	...
	public class Conexion
	{
		...
		public List<Dictionary<string, object>> GetPermisosDeTiempoEspecifico(int Id)
		{
			...
			using (SqlConnection connection = OpenConnection())
			{
				if (connection != null)
				{
					string query = "SELECT Id, IdEmpleado, FechaInicio, FechaFin, Motivo, Estado FROM PermisosDeTiempo WHERE IdEmpleado = @IdEmpleado";
					using (SqlCommand command = new SqlCommand(query, connection))
					{
						try
						{
		...
		public List<Dictionary<string, object>> GetPermisosDeAusenciaEspecifico(int Id)
		{
			var permisos = new List<Dictionary<string, object>>();
			...
			using (SqlConnection connection = OpenConnection())
			{
				if (connection != null)
				{
					string query = "SELECT Id, IdEmpleado, FechaInicio, FechaFin, Motivo, Estado FROM PermisosDeAusencia WHERE IdEmpleado = @IdEmpleado";
					using (SqlCommand command = new SqlCommand(query, connection))
					{
						try
						{
